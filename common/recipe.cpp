    ...
    for (fl = formulalist; fl != NULL; fl = fl->next) {
        for (formula = fl->items; formula != NULL; formula = formula->next) {
            for (check = formula->next; check != NULL; check = check->next)
                ...
                if (check->index == formula->index && check->cauldron == formula->cauldron && (check->tool_size == formula->tool_size)) {
...
static long recipe_find_ingredient_cost(const char *name) {
    ...
    long value = 0;
    bool found = false;
    getManager()->archetypes()->each([&value, &found, &name, &part1] (const archetype *at) {
        if (found) {
            return;
        }
    ...
    if (cp != NULL) {
        ...
        getManager()->archetypes()->each([&value, &found, &part1, &part2] (const archetype *at) {
            if (found) {
                return;
            }
...
static void build_stringlist(const char *str, char ***result_list, size_t *result_size) {
    ...
    assert(size > 0);
    *result_list = static_cast<char **>(malloc(sizeof(**result_list) * size));
    if (*result_list == NULL)
        fatal(OUT_OF_MEMORY);
    *result_size = size;
