...
void object_copy_no_speed(const object *src_ob, object *dest_ob) {
    ...
    if (src_ob->key_values != NULL) {
        ...
        for (i = src_ob->key_values; i != NULL; i = i->next) {
            key_value *new_link = static_cast<key_value *>(malloc(sizeof(key_value)));
...
#endif
                if (!op->spell_tags)
                    op->spell_tags = static_cast<tag_t *>(calloc(SPELL_TAG_SIZE, sizeof(tag_t)));
...
int object_find_multi_free_spot_within_radius(const object *ob, const object *gen, int *hx, int *hy) {
    ...
    x_array = static_cast<int8_t *>(malloc(sx*sy*sizeof(int8_t)));
    y_array = static_cast<int8_t *>(malloc(sx*sy*sizeof(int8_t)));
...
static int object_set_value_s(object *op, const char *canonical_key, const char *value, int add_key) {
    ...
    field = static_cast<key_value *>(malloc(sizeof(key_value)));
