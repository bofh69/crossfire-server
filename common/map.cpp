...
void allocate_map(mapstruct *m) {
    ...
    m->spaces = static_cast<MapSpace *>(calloc(map_size(m), sizeof(MapSpace)));
...
static shopitems *parse_shop_string(const char *input_string, const mapstruct *map) {
    ...
    p = shop_string;
    strip_endline(p);
    items = static_cast<shopitems *>(CALLOC(number_of_entries+1, sizeof(shopitems)));
    /*
...
void update_position(mapstruct *m, int x, int y) {
    ...
    FOR_MAP_PREPARE(m, x, y, tmp) {
        ...
        if (QUERY_FLAG(tmp, FLAG_NO_MAGIC))
            flags |= P_NO_MAGIC;
        if (QUERY_FLAG(tmp, FLAG_DAMNED) || object_value_set_shared(tmp, blocks_prayer))
            flags |= P_NO_CLERIC;
