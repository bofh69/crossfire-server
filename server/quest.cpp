...
static quest_state *get_new_quest_state(void) {
    quest_state *qs = static_cast<quest_state *>(calloc(1, sizeof(quest_state)));
    if (qs == NULL)
        fatal(OUT_OF_MEMORY);
    return qs;
}
...
static quest_state *get_or_create_state(quest_player *pq, sstring name) {
    ...
    if (!qs) {
        qs = static_cast<quest_state *>(calloc(1, sizeof(quest_state)));
        if (!qs)
            fatal(OUT_OF_MEMORY);
...
static quest_player *get_or_create_quest(player *pl) {
    ...
    if (!pq) {
        pq = static_cast<quest_player *>(calloc(1, sizeof(quest_player)));
        if (!pq)
            fatal(OUT_OF_MEMORY);
