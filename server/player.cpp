...
void set_player_socket(player *p, socket_struct *ns) {
    ...
    p->socket = static_cast<socket_struct *>(malloc(sizeof(socket_struct)));
    if (!p->socket) {
        fatal(OUT_OF_MEMORY);
    }
    ...
    FOR_BELOW_PREPARE(op, tmp) {
        ...
        if (!(op->contr->mode&PU_NEWMODE)) {
            ...
            default:
                ...
                && (price_base(tmp)*100/(tmp->weight*MAX(tmp->nrof, 1))) >= op->contr->mode)
                    pick_up(op, tmp);
            }
        } else { /* old model */
            /* NEW pickup handling */
            if (op->contr->mode&PU_DEBUG) {
                ...
                draw_ext_info_format(NDI_UNIQUE, 0, op, MSG_TYPE_COMMAND, MSG_TYPE_COMMAND_DEBUG,
                                     ...
                                     (int)(price_base(tmp)*100/(tmp->weight*MAX(tmp->nrof, 1))));
            ...
            if (op->contr->mode&PU_RATIO) {
                ...
                current_ratio = price_base(tmp)*100/(tmp->weight*MAX(tmp->nrof, 1));
...
void player_unready_range_ob(player *pl, object *ob) {
    ...
    for (i = 0; i < static_cast<int>(range_size); i++) {
        if (pl->ranges[i] == ob) {
            pl->ranges[i] = NULL;
            if (pl->shoottype == i) {
