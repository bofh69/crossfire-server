...
static int knowledge_add(knowledge_player *current, const char *item, const knowledge_type *kt) {
     ...
     */
    check = static_cast<knowledge_item *>(malloc(sizeof(knowledge_item)));
    check->item = add_string(item);
    check->handler = kt;
    if (current->item_count >= current->item_allocated) {
        current->item_allocated += 10;
        current->items = static_cast<knowledge_item **>(realloc(current->items, current->item_allocated * sizeof(knowledge_item*)));
    }
...
static int knowledge_god_add(struct knowledge_player *current, const char *item, const struct knowledge_type *type) {
    char *dup = strdup_local(item), *pos = const_cast<char *>(strchr(dup, ':'));
    StringBuffer *buf;
    int what, i;
    knowledge_item* check;
    ...
    *pos = '\0';
    ...
    for (i = 0; i < current->item_count; i++) {
        ...
        if (strncmp(check->item, dup, strlen(dup)) == 0) {
            ...
            int known, result;
            pos = const_cast<char *>(strchr(check->item, ':'));
            known = atoi(pos + 1);
            result = known | what;
            buf = stringbuffer_new();
...
static void knowledge_read_player_data(knowledge_player *kp) {
    ...
    while (fgets(read, sizeof(read), file) != NULL) {
        ...
        *dot = '\0';
        ...
        item = static_cast<knowledge_item *>(malloc(sizeof(knowledge_item)));
        item->item = add_string(dot + 1);
        item->handler = type;
        if (kp->item_count == kp->item_allocated) {
            kp->item_allocated += 10;
            kp->items = static_cast<knowledge_item **>(realloc(kp->items, kp->item_allocated * sizeof(knowledge_item*)));
        }
...
static knowledge_player *knowledge_get_or_create(const player *pl) {
    ...
    cur = static_cast<knowledge_player *>(calloc(1, sizeof(knowledge_player)));
    if (!cur)
        fatal(OUT_OF_MEMORY);
...
void knowledge_give(player *pl, const char *marker, const object *book) {
    ...
    dot = const_cast<char *>(strchr(marker, ':'));
    if (dot == NULL)
        return;
    ...
    dot = const_cast<char *>(strchr(copy, ':'));
    *dot = '\0';
    dot++;
