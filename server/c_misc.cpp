...
static void map_info(object *op, const char *search) {
    ...
    for (mapstruct *m = first_map; m != NULL; m = m->next) {
        ...
        if (m->players) {
            ttr = (m->unique ? 0 : m->reset_timeout) + ticks_to_seconds(MAP_MINTIMEOUT);
        }
        else if (m->timeout) {
            ttr = (m->unique ? 0 : m->reset_timeout) + ticks_to_seconds(m->timeout);
        }
...
void command_bowmode(object *op, const char *params) {
    ...
    for (i = 0, found = 0; i <= bow_bestarrow; i++) {
        if (!strcmp(params, types[i])) {
            found++;
            op->contr->bowtype = static_cast<bowtype_t>(i);
            break;
        }
    }
    if (!found) {
