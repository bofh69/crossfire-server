...
void command_setgod(object *op, const char *params) {
    ...
    if (*params == '\0' || !(str = const_cast<char *>(strchr(params, ' ')))) {
        draw_ext_info(NDI_UNIQUE, 0, op, MSG_TYPE_COMMAND, MSG_TYPE_COMMAND_ERROR,
    ...
    if (cp) {
        ...
        if (!at_spell) {
            if (find_artifactlist(at->clone.type) == NULL) {
                ...
                const auto& items = find_artifactlist(at->clone.type)->items;
...
static void reset_faces_sent(struct socket_struct *socket) {
    ...
    socket->faces_sent = static_cast<uint8_t *>(calloc(sizeof(socket->faces_sent[0]), get_faces_count()));
...
void command_reset(object *op, const char *params) {
    ...
    if (*params == '\0') {
        draw_ext_info(NDI_UNIQUE, 0, op, MSG_TYPE_COMMAND,
                MSG_TYPE_COMMAND_ERROR, "Which map should be reset?");
        return;
    }
    ...
    if (m == NULL) {
        draw_ext_info(NDI_UNIQUE, 0, op, MSG_TYPE_COMMAND,
                MSG_TYPE_COMMAND_ERROR, "No such map.");
        return;
    }
    ...
    if (confirmation) {
        if (m->unique && (op->map == m)) {
            draw_ext_info_format(NDI_UNIQUE, 0, op, MSG_TYPE_COMMAND,
                    ...
                    "'reset full-reset %s' while standing somewhere else.",
                    m->path);
            return;
        }
        ...
        if (strncmp("full-reset", confirmation, strlen("full-reset"))) {
            draw_ext_info(NDI_UNIQUE, 0, op, MSG_TYPE_COMMAND,
                    MSG_TYPE_COMMAND_ERROR, "Confirm using 'full-reset'.");
            return;
        }
    }
    ...
    if (confirmation) {
        draw_ext_info_format(NDI_UNIQUE, 0, op, MSG_TYPE_COMMAND,
                MSG_TYPE_COMMAND_DM, "Fully resetting map %s.", path);
    } else {
        draw_ext_info_format(NDI_UNIQUE, 0, op, MSG_TYPE_COMMAND,
                MSG_TYPE_COMMAND_DM, "Resetting map %s.", path);
    }
...
void do_dump(object *who, object *what) {
    ...
    object_dump(what, sb);
    diff = stringbuffer_finish(sb);
    draw_ext_info(NDI_UNIQUE, 0, who, MSG_TYPE_COMMAND, MSG_SUBTYPE_NONE, diff);
    free(diff);

    /* Let's push that item on the dm's stack */
    dm_stack_push(who->contr, what->count);
}
