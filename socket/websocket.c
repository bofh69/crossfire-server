...
/* Include global.h first so HAVE_LIBWEBSOCKETS is visible from autoconf.h */
#include <global.h>
...
static int get_free_socket_slot(void)
{
    ...
    if (socket_info.allocated_sockets <= socket_info.nconns) {
        init_sockets = realloc(init_sockets,
        ...
        init_sockets[newsocknum].faces_sent =
            calloc(1, nrofpixmaps * sizeof(*init_sockets[newsocknum].faces_sent));
...
void ws_request_write(socket_struct *ns)
{
    if (ns->is_websocket && ns->wsi)
        lws_callback_on_writable((struct lws *)ns->wsi);
}
...
void ws_cleanup_connection(socket_struct *ns)
{
    ...
    sd = (struct ws_session_data *)lws_wsi_user((struct lws *)ns->wsi);
    if (sd)
