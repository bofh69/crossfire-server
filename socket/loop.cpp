...
static void new_connection(int listen_fd) {
    ...
    if (newsocknum == -1) {
        ...
        init_sockets = static_cast<socket_struct *>(realloc(init_sockets, sizeof(socket_struct)*(socket_info.allocated_sockets+1)));
        if (!init_sockets)
            fatal(OUT_OF_MEMORY);
        ...
        init_sockets[newsocknum].wsi = nullptr;
...
void do_server(void) {
    ...
    for (int i = 0; i < socket_info.allocated_sockets; i++) {
        if (init_sockets[i].status == Ns_Dead) {
            ...
            if (init_sockets[i].is_websocket && init_sockets[i].ws_proxy_fd != -1)
                FD_SET((uint32_t)init_sockets[i].ws_proxy_fd, &tmp_read);
#endif
        }
    }
    ...
    for (pl = first_player; pl != NULL; ) {
        if (pl->socket->status == Ns_Dead) {
            ...
            if (pl->socket->is_websocket && pl->socket->ws_proxy_fd != -1)
                FD_SET((uint32_t)pl->socket->ws_proxy_fd, &tmp_read);
#endif
            pl = pl->next;
        }
    }
    ...
    while (sleep_time > 0) {
        ...
        for (int i = 0; i < socket_info.allocated_sockets; i++) {
            ...
            if (init_sockets[i].is_websocket && init_sockets[i].ws_proxy_fd != -1
                && FD_ISSET((uint32_t)init_sockets[i].ws_proxy_fd, &tmp_read))
        ...
        for (pl = first_player; pl != NULL; pl = next) {
            ...
            if (FD_ISSET(pl->socket->fd, &tmp_exceptions)) {
                ...
                if (pl->socket->is_websocket && pl->socket->ws_proxy_fd != -1
                    && FD_ISSET((uint32_t)pl->socket->ws_proxy_fd, &tmp_read))
