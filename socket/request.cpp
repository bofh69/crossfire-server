...
void account_play_cmd(char *buf, int len, socket_struct *ns)
{
    ...
    for (pl = first_player; pl; pl = pl->next) {
        if (pl->ob && pl->socket != ns && strcmp(buf, pl->ob->name) == 0 && pl->state != ST_PLAY_AGAIN && pl->state != ST_GET_NAME) {
            SockList_AddPrintf(&sl,
...
void create_player_cmd(char *buf, int len, socket_struct *ns)
{
    ...
    if (ns->login_method >= 2) {
        ...
        char *key, *value, *race=NULL, *class_name=NULL;
        ...
        while (nlen < len) {
            ...
            *value = 0;
            ...
            else if (!strcasecmp(key,"class"))   class_name = value;
        ...
        if (class_name)
            class_a = try_find_archetype(class_name);
