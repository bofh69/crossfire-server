...
it should not require a 6 hour continuous sitting, as the player works
...
It is very frustrating to play a map, killing most every monster without
...
13) Read the 'quests' file to learn how to manage quests.

14) For any special characters (accented a for example), the format
for maps and archetype is utf8.
...
easy with Gridarta, just select auto-joining and make zig-zag with mouse.
...
The server will automatically pull out face information from face files.
...
Remember that difficulty isn't only "good" for the player; traps (in
doors and chests) are also generated as treasure, and as such, they
will be more deadly the higher the difficulty.
