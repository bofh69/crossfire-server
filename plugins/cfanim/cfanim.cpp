...
static anim_move_result runsay(CFanimation *animation, long int id, void *parameters) {
    (void)id;
    if (parameters) {
        cf_object_say(animation->victim, static_cast<const char *>(parameters));
        free(parameters);
    } else
...
static long int initapplyobject(const char *name, char *parameters, CFmovement *move_entity) {
    (void)name;
    move_entity->parameters = parameters ? (void*)cf_add_string(static_cast<sstring>(parameters)) : NULL;
    return 1;
}
...
static anim_move_result runapplyobject(CFanimation *animation, long int id, void *parameters) {
    ...
    if (!current)
        current = cf_object_find_by_name(animation->victim, static_cast<char *>(parameters));
    if (!current) {
        cf_free_string(static_cast<sstring>(parameters));
        return mr_finished;
    }
    aflag = AP_APPLY;
...
static long int initdropobject(const char *name, char *parameters, CFmovement *move_entity) {
    ...
    move_entity->parameters = parameters ?
        (void *)cf_add_string(static_cast<sstring>(parameters)) : NULL;
    return 1;
}
...
static anim_move_result rundropobject(CFanimation *animation, long int id, void *parameters) {
    ...
    if (!parameters)
        return mr_finished;
    what = cf_object_find_by_name(animation->victim, static_cast<sstring>(parameters));
    if (what != NULL)
...
static  anim_move_result runnotice(CFanimation *animation, long int id, void *parameters) {
    ...
    cf_player_message(animation->victim, static_cast<char *>(parameters), val);
    return mr_finished;
}
...
static CFanimation *create_animation(void) {
    ...
    anim = (CFanimation *)malloc(sizeof(CFanimation));
    if (!anim)
        return NULL;
    anim->name = NULL;
    anim->victim = NULL;
    anim->event = NULL;
    anim->nextmovement = NULL;
    anim->nextanimation = NULL;
    anim->delete_end = 0;
    for (current = first_animation; (current && current->nextanimation); current = current->nextanimation)
        ;
    if (!current)
        first_animation = anim;
    else
        current->nextanimation = anim;
    return anim;
}
