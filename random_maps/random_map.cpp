...
char **rotate_layout(char **maze, int rotation, RMParms *RP)
{
    ...
    case 2: { /* a reflection */
        char *rotated = static_cast<char *>(malloc(sizeof(char)*RP->Xsize*RP->Ysize));

        for (i = 0; i < RP->Xsize; i++) { /* make a copy */
            for (j = 0; j < RP->Ysize; j++) {
                rotated[i*RP->Ysize+j] = maze[i][j];
            }
        }
        for (i = 0; i < RP->Xsize; i++) { /* copy a reflection back */
            for (j = 0; j < RP->Ysize; j++) {
                maze[i][j] = rotated[(RP->Xsize-i-1)*RP->Ysize+RP->Ysize-j-1];
            }
        }
        free(rotated);
        return maze;
...
void doorify_layout(char **maze, RMParms *RP)
{
    ...
    doorlist_x = static_cast<int *>(malloc(sizeof(int)*RP->Xsize*RP->Ysize));
    doorlist_y = static_cast<int *>(malloc(sizeof(int)*RP->Xsize*RP->Ysize));
